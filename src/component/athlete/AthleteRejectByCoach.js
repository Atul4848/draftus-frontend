...
import { Collapse, Skeleton, Button, Badge } from "antd";
import { useSelector, useDispatch } from "react-redux";
import { athleteRejectByCoach } from "../../redux/actions/athleteAction";
import { Link } from "react-router-dom";

function AthleteRejectByCoach() {
  const { Panel } = Collapse;

  const dispatch = useDispatch();
  const athlete_detail = useSelector((state) => state.athlete_detail);
  const { athlete_Rejected_By_Coach, loading } = athlete_detail;

  useEffect(() => {
    dispatch(athleteRejectByCoach());
  }, []);
        ...
        <>
          ...
          <div className="coachesflied">
            {athlete_Rejected_By_Coach == "" ? (
              ...
              <Collapse ghost>
                {athlete_Rejected_By_Coach.map((item) => (
                  <Panel
                    header={
                      <span>
                        <span>
                    ...
                    key={item ? item.id : "1"}
                  >
                    <Badge.Ribbon text="you can request after 3 month">
                      <>
...
export default AthleteRejectByCoach;
